...
ax.legend()
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "plot_01_count_distribution.png"), dpi=300)
plt.close()
...
ci_nb  = np.exp(np.asarray(nb_fit.conf_int())[:-1])
print("\nIRR (NB):")
for i, name in enumerate(["intercept", *pred_cols]):
    p_val = nb_fit.pvalues[i]
    print(f"  {name}: IRR = {irr_nb[i]:.3f}, "
          f"95% CI [{ci_nb[i, 0]:.3f}, {ci_nb[i, 1]:.3f}], p {fmt_p(p_val)}")
...
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "plot_06_unified_importance.png"), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "plot_08_rate_distribution.png"), dpi=300)
plt.close()
...
ci_nr  = np.exp(np.asarray(nb_rate.conf_int())[:-1])
print("\nIRR (NB rate model):")
for i, name in enumerate(["intercept", *pred_cols]):
    print(f"  {name}: IRR = {irr_nr[i]:.3f}, "
          f"95% CI [{ci_nr[i, 0]:.3f}, {ci_nr[i, 1]:.3f}], p {fmt_p(nb_rate.pvalues[i])}")
print()
