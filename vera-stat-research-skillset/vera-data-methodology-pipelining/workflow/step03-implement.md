...
Read `reference/specs/implementation-tracks.md` for detailed track specifications.
Read `reference/specs/simulation-standards.md` for simulation code requirements.
Read `reference/patterns/proof-patterns.md` for proof structure patterns.
...
All active tracks launch using the parallel-worker surface bound at pipeline
start (see SKILL.md "Runtime Capability Detection"). `DISPATCH_MODE` governs
how the tracks are launched:

- `Agent` / `Task` — emit multiple independent tool calls in one response.
- `spawn_agent` + `send_input` + `wait_agent` — spawn each track, send its
  prompt, wait for completion, collect results before 3.3 convergence.
- `sequential` — run tracks one after another in the main agent loop. Track
  A, then Track B, then Track C. Slower but produces the same artifacts;
  log `dispatch_mode="sequential"` to RESEARCH_LOG.md.

The pipeline NEVER aborts because a specific parallel-worker tool is missing —
always pick the first available option, degrading to sequential if needed.

### 3.2 Launch Tracks (parallel or sequential per `DISPATCH_MODE`)
