---
name: vera-stat-multivariate-analyzing
description: >-
  Server-side extension that completes the full analysis pipeline for
  multiple continuous outcome variables after vera-stat-multivariate-testing
  has run. Adds follow-up univariate ANOVAs with Bonferroni-corrected
  pairwise comparisons, full discriminant function analysis with
  classification accuracy, MANCOVA and two-way MANOVA, profile analysis
...
Continues from where vera-stat-multivariate-testing stopped (PART 0-2 done).
...
Same as vera-stat-multivariate-testing, plus:
...
Invoked directly after `vera-stat-multivariate-testing` or orchestrated by `vera-stat-application-pipeline`.
