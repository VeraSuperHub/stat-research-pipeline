...
```
REPO_ROOT must point to the directory that contains the flat sibling skill
folders, for example both `vera-data-continuous-reviewing/` and
`vera-data-continuous-generating/`. The paths stored in PIPELINE_STATE.json
(testing_skill_path, analysis_skill_path) are written relative to this root:

  testing_skill_path  = "vera-data-continuous-reviewing"
  analysis_skill_path = "vera-data-continuous-generating"

REPO_ROOT discovery (in priority order):
1. Check STATRESEARCH_ROOT environment variable.
2. Resolve relative to THIS skill file: ../../
3. Walk upward from PWD looking for a directory that contains both resolved
   sibling skill folders.

TESTING_SKILL_PATH = {REPO_ROOT}/{testing_skill_path}
ANALYZING_SKILL_PATH = {REPO_ROOT}/{analysis_skill_path}

Validation (abort with clear error if any fail):
- `{TESTING_SKILL_PATH}/SKILL.md` must exist.
- `{ANALYZING_SKILL_PATH}/SKILL.md` must exist.
If validation fails, emit: "REPO_ROOT={REPO_ROOT} does not contain the routed
reviewing/generating skill pair. Set STATRESEARCH_ROOT explicitly, or invoke
the pipeline from inside the extracted skillset tree."
...
vary by outcome type (see reference/specs/method-tracks.md).

### 4.2 Launch Streams Using Dependency Graph

Read `method_tracks` from PIPELINE_STATE.json. Partition into:
- **Independent tracks**: `depends_on` is null → launch immediately via the bound parallel-worker surface
...
in order (T1→T2→T3→...) rather than spawning workers.

Launch ALL independent work using the parallel-worker surface bound at Stage 4
start per `DISPATCH_MODE` (see SKILL.md "Tool Usage — Parallel-worker surface").
Probe tool availability in this priority order and bind the first one that
resolves:

- `Agent` or `Task` (Claude Code / Claude Agent SDK) — emit multiple
  independent tool calls in a single response.
- `spawn_agent` + `send_input` + `wait_agent` (Codex / lifecycle-based SDKs) —
  use only when the runtime policy and user authorization permit subagents;
  spawn each independent track, send its prompt, wait for completion, collect
  results before 4.5 convergence.
- `sequential` fallback — if NO parallel-worker tool is available, run
  Stream A first, then each independent Stream-B track one after another in
  the main agent loop. Slower but produces the same artifacts. Log
  `dispatch_mode="sequential"` in RESEARCH_LOG.md and PIPELINE_STATE.json.

The pipeline NEVER aborts because a specific worker tool is missing —
sequential fallback always completes.
...
Follow reference/patterns/literature-reviewing.md to search across arXiv, Google Scholar, Semantic Scholar, and statistics journals.
