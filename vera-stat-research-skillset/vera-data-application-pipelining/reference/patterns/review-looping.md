...
Autonomously iterate: review -> implement fixes -> re-review, until the reviewer
gives a positive assessment or MAX_ROUNDS is reached. External review uses a
configured reviewer bridge when one is available; otherwise the caller should use
the self-review fallback in `workflow/step07-review.md`.
...
- REVIEWER_MODEL = `gpt-5.4` — Default model when the configured reviewer bridge supports model selection
...
Send comprehensive context to the configured external reviewer bridge:
...
If this is round 2+, continue the same reviewer thread when the bridge supports
threaded replies. With Codex MCP, use `mcp__codex__codex-reply` with the saved
threadId.
...
- Save the reviewer thread ID from the first call when the bridge supports threaded replies
