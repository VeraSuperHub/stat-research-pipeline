...
Read `PAPER_PLAN.md` for the outline. If it doesn't exist, ask the user to run `/paper-plan` first.
...
Send the complete draft to the configured reviewer bridge when available; if no
bridge is available, perform the same checklist as a self-review pass:
