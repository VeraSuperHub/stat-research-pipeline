...
```mermaid
graph TD
    T1[T1_primary<br/>distribution + primary test]
    T2[T2_regression<br/>OLS / GLM]
    T3[T3_trees<br/>CART / RF / LightGBM]
    T4[T4_qr<br/>quantile / alternative]
    T5[T5_subgroup<br/>stratified + interaction]
    CONV[Convergence<br/>step 07 cross-method]
    ASM[Assembly<br/>step 08 manuscript]
...
**Parallelism rules:**
- T1, T2, T3, T4 run in parallel at Stage 4 start (T2/T3/T4 do not depend on T1 output, only on raw data; the testing-skill artifacts are used only by T5 and by the final convergence narrative).
- T5 waits for T1 (needs primary-test output to build stratified comparison).
- Convergence (step 07) waits for all five tracks.
- Assembly (step 08) always last.

If a track fails, downstream tracks that depend on it are marked `stranded` in `PIPELINE_STATE.json`; independent tracks continue. The manuscript reports the gap explicitly.
