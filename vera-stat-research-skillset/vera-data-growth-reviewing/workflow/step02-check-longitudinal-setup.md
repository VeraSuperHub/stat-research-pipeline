...
if research_question is "overall trajectory shape" or "rate of change":
    ...
    model = "latent-basis (freed-loading) growth model"  # also called "freed-loading" or "free-curve" in some sources
    # first and last loadings fixed (typically 0 and 1); intermediate loadings freely estimated,
    # interpreted as the proportion of total change completed at each wave.
    note: "Start with linear; test latent-basis (freed-loading) as follow-up"
```
